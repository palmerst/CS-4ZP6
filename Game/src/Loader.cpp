...
bool checkSuccess(funcParam getParams, funcErr getError, GLuint handle, GLenum check){
    int status;
    getParams(handle, check, &status);
    if(status)
        return true;
    int errorLength;
...
GLuint compileShader(const char* sourceFileName, GLenum shaderType){
    ...
    checkSuccess(glGetShaderiv, glGetShaderInfoLog, shaderHandle, GL_COMPILE_STATUS);
...
GLuint loadShaders(const char* vShader, const char* fShader){
	...
	checkSuccess(glGetProgramiv, glGetProgramInfoLog, programHandle, GL_LINK_STATUS);
