...
vec3 phongModel(vec3 n, vec3 color, mat3 TBN){
    vec3 spec = vec3(0.0);
...
void main()
{
    ...
    vec3 texNormal = normalize(texture(Bump, UVadj).rgb*2.0 - 1.0);
	
	//finalColor = vec4(phongModel(texColor.rgb), 1.0);
    finalColor = vec4(phongModel(texNormal, texColor.rgb, TBN), 1.0);
}